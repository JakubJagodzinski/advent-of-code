...
def count_trailhead_score(map, previous_height, column_nr, row_nr, visited_map):
    ...
    if map[row_nr][column_nr] == TRAILHEAD_END_HEIGHT:
        ...
        return 1
    score = 0
    score += count_trailhead_score(map, map[row_nr][column_nr], column_nr, row_nr - 1, visited_map)
    score += count_trailhead_score(map, map[row_nr][column_nr], column_nr - 1, row_nr, visited_map)
    score += count_trailhead_score(map, map[row_nr][column_nr], column_nr + 1, row_nr, visited_map)
    score += count_trailhead_score(map, map[row_nr][column_nr], column_nr, row_nr + 1, visited_map)
    return score
    ...
    for row_nr in range(len(map)):
        for col_nr in range(len(map[row_nr])):
            if map[row_nr][col_nr] == 0:
                ...
                ranks.append(count_trailhead_score(map, -1, col_nr, row_nr, visited_map))
...
if __name__ == '__main__':
    map = load_map()
    scores, ranks = count_trailheads_scores(map)
    print(scores)
    print(sum(scores))
    print(ranks)
    print(sum(ranks))
